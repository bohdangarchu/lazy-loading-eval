...
from shared.registry import zstd_base_image, tdfs_cmd, registry
...
def build_only(n: int, cfg: EnvConfig, source_image: str = "") -> BuildResult:
    target = f"{registry(cfg)}/build-perf-stargz-zstd:{n}"
    ...
    log.info(f"=== Building with {n} split(s) (2dfs-stargz-zstd) ===")
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark tdfs build --enable-stargz --use-zstd across split counts")
