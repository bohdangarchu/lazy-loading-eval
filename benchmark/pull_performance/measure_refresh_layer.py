...
from pull_performance.paths import (
    refresh_layer_csv_path, refresh_layer_chart_path, refresh_layer_artifacts_dir,
)
...
def _build_name_refresh_layer(source_image: str, cfg, mode: str, version_idx: int) -> str:
    return f"{registry(cfg)}/{image_slug(source_image)}-{mode}-refresh-layer:v{version_idx}"


def _pull_name_refresh_layer(source_image: str, cfg, mode: str, version_idx: int, max_allowed_splits: int) -> str:
    end_col = max_allowed_splits - 1
    return f"{registry(cfg)}/library/{image_slug(source_image)}-{mode}-refresh-layer:v{version_idx}--0.0.0.{end_col}"
...
def _build_version(
    ...
    target = _build_name_refresh_layer(source_image, cfg, mode, version_idx)
    ...
    log.info(f"Building {mode} refresh-layer v{version_idx}: {target}")
    ...
    for mode in MODES:
        ...
        for run in range(CFG.refresh_layer_n_runs):
            ...
            for pct in PARTITION_PERCENTS:
                ...
                v0_pull = _pull_name_refresh_layer(source_image, cfg, mode, 0, max_allowed_splits)
                ...
                name = _next_container_name(f"refresh-layer-{mode.replace('-', '')}")
...
def print_results(results: dict[str, list[tuple[int, int, float, float, float]]]) -> None:
    log.result(f"\n=== Refresh-Layer Performance Results (mean ± stddev, n={CFG.refresh_layer_n_runs} runs) ===")
    log.result(
        f"{'pct':>5}  {'mode':<32}  "
...
def save_results_csv(
    ...
    path = refresh_layer_csv_path(SCRIPT_DIR, model, base_image, execution_ts)
...
def plot(
    ...
    ax.set_title(f"refresh-layer time (mean ± stddev, n={CFG.refresh_layer_n_runs} runs)")
    ax.set_xticks(x)
    ...
    output_path = refresh_layer_chart_path(SCRIPT_DIR, model, base_image, execution_ts)
...
def main():
    ...
    for model, base_image, max_allowed_splits in EXPERIMENTS:
        ...
        for mode in MODES:
            log.info(f"\n=== Preparing mode: {mode} ===")
            artifacts_dir = refresh_layer_artifacts_dir(SCRIPT_DIR, execution_ts, model, base_image, build_mode(mode))
            all_digests_per_mode[mode] = prepare_refresh_layer(chunk_paths, base_image, CFG, mode, max_allowed_splits, artifacts_dir)

        results = measure_refresh_layer(base_image, CFG, all_digests_per_mode, max_allowed_splits)
