...
from pull_performance.paths import (
    refresh_full_artifacts_dir, refresh_full_chart_path,
    refresh_full_csv_path, refresh_full_log_path,
)
...
def _build_target(version: int) -> str:
    return f"{registry(CFG)}/{image_slug(SOURCE_IMAGE)}-{MODE}-refresh-model:v{version}"


def _pull_ref(version: int) -> str:
    return f"{registry(CFG)}/library/{image_slug(SOURCE_IMAGE)}-{MODE}-refresh-model:v{version}--0.0.0.0"
...
def _repo() -> str:
    return f"library/{image_slug(SOURCE_IMAGE)}-{MODE}-refresh-model"
...
def _setup_warm_v0(in_paths: list[str]) -> str:
    ...
    name = _next_container_name(f"refresh-model-v0")
...
def _run_baseline_arm(
    ...
    name2 = _next_container_name("refresh-model-v1")
    t0 = time.perf_counter()
...
def measure_refresh_model(snapshot_files: list[str], execution_ts: str) -> dict:
    """results = {
    ...
    for run in range(N_RUNS):
        ...
        baseline_log = refresh_full_log_path(
            SCRIPT_DIR, MODEL, SOURCE_IMAGE, "baseline", run, execution_ts,
        )
        refresh_log = refresh_full_log_path(
...
def save_results_csv(results: dict, execution_ts: str) -> None:
    path = refresh_full_csv_path(SCRIPT_DIR, MODEL, SOURCE_IMAGE, execution_ts)
...
def plot(results: dict, execution_ts: str) -> None:
    ...
    output_path = refresh_full_chart_path(SCRIPT_DIR, MODEL, SOURCE_IMAGE, execution_ts)
...
def refresh_model_main(execution_ts: str) -> None:
    ...
    artifacts_dir = refresh_full_artifacts_dir(
    ...
    snapshot_files = prepare_refresh_model(
        artifacts_dir=artifacts_dir, toc_dir=toc_dir,
    )

    results = measure_refresh_model(snapshot_files, execution_ts)
...
if __name__ == "__main__":
    refresh_model_main(datetime.now(timezone.utc).strftime("%Y-%m-%dT%H-%M-%SZ"))
